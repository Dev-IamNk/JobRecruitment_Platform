<!-- FILE: pages/take_test.php (DEBUG VERSION) -->
<?php 
...
echo "<!-- Debug: Generate test loaded -->\n";

redirectIfNotLoggedIn();
echo "<!-- Debug: Login checked -->\n";

if (getUserType() != 'candidate') {
    die("Error: Not logged in as candidate");
}

$application_id = intval($_GET['app_id'] ?? 0);
echo "<!-- Debug: Application ID = $application_id -->\n";

$candidate_id = $_SESSION['user_id'];
echo "<!-- Debug: Candidate ID = $candidate_id -->\n";

// Verify this application belongs to logged-in candidate
$stmt = $pdo->prepare("SELECT a.*, j.title as job_title, j.id as job_id FROM applications a JOIN jobs j ON a.job_id = j.id WHERE a.id = ? AND a.candidate_id = ?");
$stmt->execute([$application_id, $candidate_id]);
$application = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$application) {
    die("Error: Application not found or doesn't belong to you. App ID: $application_id, Your ID: $candidate_id");
}

echo "<!-- Debug: Application found - Job: " . $application['job_title'] . " -->\n";
...
echo "<!-- Debug: Test configs count = $test_count -->\n";

if ($test_count == 0) {
    die("Error: No test configured for this job. Please contact the recruiter.");
}
...
if ($existing_attempt) {
    echo "<!-- Debug: Existing attempt found - Status: " . $existing_attempt['status'] . " -->\n";
    
    if ($existing_attempt['status'] == 'completed') {
        die("Error: You have already completed this test. Score: " . $existing_attempt['total_score'] . "%");
    }
}

// Generate test
echo "<!-- Debug: Starting test generation... -->\n";
...
echo "<!-- Debug: Test generated successfully -->\n";
echo "<!-- Debug: Total questions = " . $test_data['total_questions'] . " -->\n";
echo "<!-- Debug: Sections = " . count($test_data['test_structure']) . " -->\n";

// Create test attempt if not exists
if (!$existing_attempt) {
    echo "<!-- Debug: Creating new test attempt... -->\n";
    ...
    if ($test_config) {
        ...
        echo "<!-- Debug: Test attempt created - ID: $attempt_id -->\n";
        ...
        echo "<!-- Debug: Saved $question_count question mappings -->\n";
    } else {
        die("Error: Test configuration not found");
    }
}
...
if ($test_json === false) {
    die("Error: Failed to encode test data to JSON - " . json_last_error_msg());
}

echo "<!-- Debug: Test data converted to JSON successfully -->\n";
echo "<!-- Debug: Ready to display test -->\n";
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="debug-info">
        ...
        Application ID: <?php echo $application_id; ?><br>
        Job: <?php echo htmlspecialchars($application['job_title']); ?><br>
        Total Questions: <?php echo $test_data['total_questions']; ?><br>
        Total Time: <?php echo $test_data['total_time']; ?> minutes<br>
        Timer Type: <?php echo $test_data['timer_type']; ?><br>
        <button onclick="this.parentElement.style.display='none'">Hide Debug Info</button>
    </div>
    ...
    <script>
        ...
        testData.test_structure.forEach(section => {
            console.log("Processing section:", section.section_name, "Questions:", section.questions.length);
        ...
        function selectAnswer(questionId, answer) {
            console.log("Answer selected - Question ID:", questionId, "Answer:", answer);
        ...
        function submitTest() {
            ...
            console.log("Submitting test - Answered:", answered, "Total:", allQuestions.length);
